...
contract Marketplace is ReentrancyGuard, KeeperCompatible, Ownable {
    ...
    function makeBidOnItem(uint256 _itemId) external payable nonReentrant {
        ...
        require(
            ...
            "Seller is not allowed to make bids"
        );
        require(!item.sold, "Item not for sale anymore");
        if (item.lastBid.amount == 0) {
            // First bid
            require(
                msg.value >= item.bidStartPrice,
                "Amount provided is lower than minimum bid price"
            );
        } else {
            // Subsequent bids
            require(
                msg.value > item.lastBid.amount,
                "Amount provided is lower or equal to last bid"
            );
        }
    ...
    function _refundLastBid(Item storage item) private returns (bool) {
        ...
        // Interactions
        (bool success, ) = payable(item.lastBid.bidder).call{
            value: item.lastBid.amount
        }(""); // Full refund from last bid
        if (success) {
            return success;
        } else {
    ...
    function _processBuyout(uint256 _itemId, uint256 _amount) private {
        ...
        require(
            _amount >= item.buyoutPrice,
            "Not enough funds to cover for price and market fee"
        );
        require(!item.sold, "Item not for sale anymore");
        // Effects
        item.sold = true;
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        for (uint256 i = 0; i < itemsForSale.length; i++) {
            if (
                block.timestamp - items[itemsForSale[i]].auctonStartTimestamp >=
                items[itemsForSale[i]].auctionDuration
            ) {
